...
#   3.) db_path : path to blast database and prefix of database name
...
seq_type <- 'AA'
db_path  <- "~/Desktop/Blast/mm.ref.prot"
...
for(i in 1:n){
    ...
    bl_results <- predict(object     = blast_db, 
                          newdata    = seq_ss[i,],
                          BLAST_args = '-comp_based_stats 0 -evalue 1000000 -word_size 2 -threshold 2')  
